%% Linear Mixed Effect (LME) Model to check for the effect of mPFC inactivation on the absolute bias (PR2)

% Our dependent Variable,  Pr2 (or Pr1, depending on the subject's Schedule),
% our fixed effect variable, Laser ON and Pheno; our random effect,  Rat ID.

%                                                       Luis de la Cuesta 02/2025

clear all
Condition = 2; % 1 Reward probabilities, 2 Stimulus presentation probabilities (SPP),3 PUN
...
for iDataset=1:length(RowsOfInterest)
    ...
    for iSess = 1:max(SessRow) % for all sessions
        ...
        for jSubBlock = 1:max(allBData(ThisSessionTrials,16)) % for all blocks in this session
    ...
    if Punishement ~=1
        AbsBiasMatrix = AllPR2_Sess(:,SubBlocksofInterest);
        if Schedule==1
            AbsBiasMatrix = AbsBiasMatrix;
        elseif Schedule==2
            AbsBiasMatrix = 1-AbsBiasMatrix;
        end
    elseif Punishement ==1
        if Schedule==1
            AbsBiasMatrix(:,1)   = 1-AllPR2_Sess(:,SubBlocksofInterest(1));
            AbsBiasMatrix(:,2)   = AllPR2_Sess(:,SubBlocksofInterest(2));
        elseif Schedule==2
            AbsBiasMatrix(:,1)   = AllPR2_Sess(:,SubBlocksofInterest(1));
            AbsBiasMatrix(:,2)   = 1-AllPR2_Sess(:,SubBlocksofInterest(2));
        end
    end
