%% Block_trials_PR2_and_RT_x_subject
% PLot the data PR2 and RT for a given subject (animalID). PLots
...
if Punishement==1
    ...
    ProgrammedFirstTrialInBlock    = [1,  121,  361, 481, 741 ];                  % Blocks are session divisions in which  an experimental variable is manipulated (in this case we manipulate RP1 and RP2 blockwise)
...
ax = arrayfun( @(i) subplot(3,2,i,'NextPlot','add','Box','on'), [1:6] );
...
%p2 = arrayfun( @(i) plot( ax(1), SubBlockxaxis, MeanRewardRates(1,:),'-g','LineWidth',3), [1:2] );                       % Plot mean Withdrawal rate
arrayfun( @(i) area( ax(1), Shading_x(1,:), Shading_y(1,:),'FaceColor','y','FaceAlpha',.1) , [1:2]);                    % Plot shading stimulated block
if Punishement
    arrayfun( @(i) axis( ax(1), [xaxis 0.20 0.80]), [1:2]);
else
...
%arrayfun( @(i) plot( ax(2), SubBlockxaxis, MeanRewardRates(2,:),'-g','LineWidth',3), [1:2] );                        % Plot mean Withdrawal rate
arrayfun( @(i) area( ax(2), Shading_x(2,:), Shading_y(1,:),'FaceColor','y','FaceAlpha',.1) , [1:2]);                     % Plot shading stimulated block
if Punishement
    arrayfun( @(i) axis( ax(2), [xaxis 0.20 0.80]), [1:2]);
else
...
arrayfun( @(i) xline( ax(3), BlockChanges(i)+0.5,'--k'), [1:length(BlockChanges)-1] );                                   % Plot dprimes for Block 3 stimulation
arrayfun( @(i) plot( ax(3), SubBlockxaxis, AllRewardRates_Sess((WichBlockIsStimulated==OptoInBlocks(1)),:),'color', black ), [1:2] ); % Plot individual sessions
p3 = arrayfun( @(i) plot( ax(3), SubBlockxaxis, MeanRewardRates(1,:),'-k','LineWidth',3), [1:2] );
arrayfun( @(i) plot( ax(3), SubBlockxaxis, AllWithdrawalRates_Sess((WichBlockIsStimulated==OptoInBlocks(1)),:),'-g' ), [1:2] ); % Plot individual sessions
p4 = arrayfun( @(i) plot( ax(3), SubBlockxaxis, MeanWithdrawalRates(1,:),'-g','LineWidth',3), [1:2] );
arrayfun( @(i) area( ax(3), Shading_x(1,:), Shading_y(2,:),'FaceColor','y','FaceAlpha',.1) , [1:2]);                     % Plot shading stimulated block
arrayfun( @(i) axis (ax(3),[xaxis 0 1]), [1:2]);

ylabel(ax(3),'Prob.')
xlabel(ax(3),'Trial Nr')

legend (ax(3),[p3(1) p4(1)], 'Reward','Abort','Location','southwest','FontSize',14)


arrayfun( @(i) xline( ax(4), BlockChanges(i)+0.5,'--k'), [1:length(BlockChanges)-1] );    
arrayfun( @(i) plot( ax(4), SubBlockxaxis, AllRewardRates_Sess((WichBlockIsStimulated==OptoInBlocks(2)),:),'color', black ), [1:2] ); % Plot individual sessions
arrayfun( @(i) plot( ax(4), SubBlockxaxis, MeanRewardRates(2,:),'-k' ,'LineWidth',3), [1:2] );
arrayfun( @(i) plot( ax(4), SubBlockxaxis, AllWithdrawalRates_Sess((WichBlockIsStimulated==OptoInBlocks(2)),:),'-g' ), [1:2] ); % Plot individual sessions
arrayfun( @(i) plot( ax(4), SubBlockxaxis, MeanWithdrawalRates(2,:),'-g','LineWidth',3), [1:2] );
arrayfun( @(i) area( ax(4), Shading_x(2,:), Shading_y(2,:),'FaceColor','y','FaceAlpha',.1) , [1:2]);                     % Plot shading stimulated block
arrayfun( @(i) axis (ax(4),[xaxis 0 1]), [1:2]);

xlabel(ax(4),'Trial Nr')

arrayfun( @(i) xline( ax(5), BlockChanges(i)+0.5,'--k'), [1:length(BlockChanges)-1] );                                   % Plot dprimes for Block 5 stimulation
arrayfun( @(i) plot( ax(5), SubBlockxaxis, AllResponseTimeR1((WichBlockIsStimulated==OptoInBlocks(1)),:),'color', blue ), [1:2] ); % Plot individual sessions
arrayfun( @(i) plot( ax(5), SubBlockxaxis, MeanResponseTimeR1(1,:),'-','color',blue ,'LineWidth',3), [1:2] );
arrayfun( @(i) plot( ax(5), SubBlockxaxis, AllResponseTimeR2((WichBlockIsStimulated==OptoInBlocks(1)),:),'color', orange ), [1:2] ); % Plot individual sessions
arrayfun( @(i) plot( ax(5), SubBlockxaxis, MeanResponseTimeR2(1,:),'-','color',orange ,'LineWidth',3), [1:2] );
arrayfun( @(i) area( ax(5), Shading_x(1,:), Shading_y(1,:),'FaceColor','y','FaceAlpha',.1) , [1:2]);                     % Plot shading stimulated block

arrayfun( @(i) axis (ax(5),[xaxis 0.25 1]), [1:2]);

xlabel(ax(5),'Trial Nr')


arrayfun( @(i) xline( ax(6), BlockChanges(i)+0.5,'--k'), [1:length(BlockChanges)-1] );                                   % Plot dprimes for Block 5 stimulation
arrayfun( @(i) plot( ax(6), SubBlockxaxis, AllResponseTimeR1((WichBlockIsStimulated==OptoInBlocks(2)),:),'color', blue ), [1:2] ); % Plot individual sessions
%pR1 = arrayfun( @(i) plot( ax(6), SubBlockxaxis, MeanResponseTimeR1(2,:),'-','color',blue ,'LineWidth',3), [1:2] );
pR1 =plot( ax(6), SubBlockxaxis, MeanResponseTimeR1(2,:),'-','color',blue ,'LineWidth',3);

arrayfun( @(i) plot( ax(6), SubBlockxaxis, AllResponseTimeR2((WichBlockIsStimulated==OptoInBlocks(2)),:),'color', orange ), [1:2] ); % Plot individual sessions
%pR2 = arrayfun( @(i) plot( ax(6), SubBlockxaxis, MeanResponseTimeR2(2,:),'-','color',orange ,'LineWidth',3), [1:2] );
pR2 = plot( ax(6), SubBlockxaxis, MeanResponseTimeR2(2,:),'-','color',orange ,'LineWidth',3);

arrayfun( @(i) area( ax(6), Shading_x(2,:), Shading_y(2,:),'FaceColor','y','FaceAlpha',.1) , [1:2]);                     % Plot shading stimulated block
legend(ax(5),[pR1,pR2],{'RT R1','RT R2'})

arrayfun( @(i) axis (ax(6),[xaxis 0.25 1]), [1:2]);

xlabel(ax(6),'Trial Nr')


%% Plot 2 (plot individual blocks, overwrite opto and non-opto data)
...
% %Block2 (Before Opto)
% 
% ax = arrayfun( @(i) subplot(1,4,i,'NextPlot','add','Box','on'), [1:4] );
% arrayfun( @(i) xline( ax(1), Block_relevant_changes_B2(i)+0.5,'--k'), [1:length(Block_relevant_changes_B2)] );                                  % Plot PR2 for Block 3 stimulation
% arrayfun( @(i) plot( ax(1), SubBlockxaxis(Block_2_datapoints), AllPR2_Sess((WichBlockIsStimulated==OptoInBlocks(1)),Block_2_datapoints),"--",'color', [0.8500 0.3250 0.0980],'LineWidth',0.25 ), [1] )     % Plot individual sessions LASER ON
% p1 = arrayfun( @(i) plot( ax(1), SubBlockxaxis(Block_2_datapoints), MeanPR2(1,Block_2_datapoints),'color',[0.8500 0.3250 0.0980],'LineWidth',3), [1] );                                              % Plot mean PR2 LASER ON
% arrayfun( @(i) plot( ax(1), SubBlockxaxis(Block_2_datapoints), AllPR2_Sess((WichBlockIsStimulated==OptoInBlocks(2)),Block_2_datapoints),"--",'color', [0,0,0],'LineWidth',0.05 ), [1] )     % Plot individual sessions LASER OFF
% p2 = arrayfun( @(i) plot( ax(1), SubBlockxaxis(Block_2_datapoints), MeanPR2(2,Block_2_datapoints),'color',[0,0,0],'LineWidth',3), [1] );                                              % Plot mean PR2 LASER OFF
% if Punishment
%     arrayfun( @(i) axis( ax(1), [[SubBlockxaxis(Block_2_datapoints(1)),SubBlockxaxis(Block_2_datapoints(end))] 0.20 0.80]), [1]);
% else
%     arrayfun( @(i) axis( ax(1), [[SubBlockxaxis(Block_2_datapoints(1)),SubBlockxaxis(Block_2_datapoints(end))] 0.00 1.00]), [1]);
% end
...
% b = get(gca,'YTickLabel');
% ylabel(ax(1),'PR2','fontsize',12)
% title (ax(1),'Block 2- Before opto')
% axis square
% axis (ax(1),'square')
% 
% %Block3 (1st opto inactivation)
% 
% arrayfun( @(i) xline( ax(2), Block_relevant_changes_B3(i)+0.5,'--k'), [1:length(Block_relevant_changes_B3)] );                                  % Plot PR2 for Block 3 stimulation
% arrayfun( @(i) plot( ax(2), SubBlockxaxis(Block_3_datapoints), AllPR2_Sess((WichBlockIsStimulated==OptoInBlocks(1)),Block_3_datapoints),"--",'color', [0.9294,0.6941,0.1255],'LineWidth',0.25 ), [1] )     % Plot individual sessions LASER ON
% p1 = arrayfun( @(i) plot( ax(2), SubBlockxaxis(Block_3_datapoints), MeanPR2(1,Block_3_datapoints),'color',[0.9294,0.6941,0.1255],'LineWidth',3), [1] );                                              % Plot mean PR2 LASER ON
% arrayfun( @(i) plot( ax(2), SubBlockxaxis(Block_3_datapoints), AllPR2_Sess((WichBlockIsStimulated==OptoInBlocks(2)),Block_3_datapoints),"--",'color', [0,0,0],'LineWidth',0.05 ), [1] )     % Plot individual sessions LASER OFF
% p2 = arrayfun( @(i) plot( ax(2), SubBlockxaxis(Block_3_datapoints), MeanPR2(2,Block_3_datapoints),'color',[0,0,0],'LineWidth',3), [1] );                                              % Plot mean PR2 LASER OFF
% if Punishment
%     arrayfun( @(i) axis( ax(2), [[SubBlockxaxis(Block_3_datapoints(1)),SubBlockxaxis(Block_3_datapoints(end))] 0.20 0.80]), [1]);
% else
%     arrayfun( @(i) axis( ax(2), [[SubBlockxaxis(Block_3_datapoints(1)),SubBlockxaxis(Block_3_datapoints(end))] 0.00 1.00]), [1]);
% end
% arrayfun( @(i) xticks(ax(2), 0:200:800), [1]);
% 
% if LabelConditions
%     arrayfun( @(i) text( ax(2),ProgrammedFirstTrialInBlock(1),0.9*ones(1,length(ProgrammedFirstTrialInBlock(1))),BlockContingenciesString(1),'FontSize',14), [1:2]); % adding condition labels in the top. One line for the (1) first, (2) middle and (3) last
%     arrayfun( @(i) text( ax(2),ProgrammedFirstTrialInBlock(2:end-1)+60,0.9*ones(1,length(ProgrammedFirstTrialInBlock(2:end-1))),BlockContingenciesString(2:end-1),'FontSize',14), [1:2]);
...
% a = get(gca,'XTickLabel');
% b = get(gca,'YTickLabel');
% ylabel(ax(2),'PR2','fontsize',12)
% title (ax(2),'Block 3- 1st opto block')
% axis square
% axis (ax(2),'square')
% 
% 
% %Block4 (After Opto)
% 
% arrayfun( @(i) xline( ax(3), Block_relevant_changes_B4(i)+0.5,'--k'), [1:length(Block_relevant_changes_B4)] );                                  % Plot PR2 for Block 3 stimulation
% arrayfun( @(i) plot( ax(3), SubBlockxaxis(Block_4_datapoints), AllPR2_Sess((WichBlockIsStimulated==OptoInBlocks(1)),Block_4_datapoints),"--",'color', [0.8500 0.3250 0.0980],'LineWidth',0.25 ), [1] )     % Plot individual sessions LASER ON
% p1 = arrayfun( @(i) plot( ax(3), SubBlockxaxis(Block_4_datapoints), MeanPR2(1,Block_4_datapoints),'color',[0.8500 0.3250 0.0980],'LineWidth',3), [1] );                                              % Plot mean PR2 LASER ON
% arrayfun( @(i) plot( ax(3), SubBlockxaxis(Block_4_datapoints), AllPR2_Sess((WichBlockIsStimulated==OptoInBlocks(2)),Block_4_datapoints),"--",'color', [0,0,0],'LineWidth',0.05 ), [1] )     % Plot individual sessions LASER OFF
% p2 = arrayfun( @(i) plot( ax(3), SubBlockxaxis(Block_4_datapoints), MeanPR2(2,Block_4_datapoints),'color',[0,0,0],'LineWidth',3), [1] );                                              % Plot mean PR2 LASER OFF
% if Punishment
%     arrayfun( @(i) axis( ax(3), [[SubBlockxaxis(Block_4_datapoints(1)),SubBlockxaxis(Block_4_datapoints(end))] 0.20 0.80]), [1]);
% else
%     arrayfun( @(i) axis( ax(3), [[SubBlockxaxis(Block_4_datapoints(1)),SubBlockxaxis(Block_4_datapoints(end))] 0.00 1.00]), [1]);
% end
% arrayfun( @(i) xticks(ax(3), 0:200:800), [1]);
% 
% if LabelConditions
...
% a = get(gca,'XTickLabel');
% b = get(gca,'YTickLabel');
% ylabel(ax(3),'PR2','fontsize',12)
% title (ax(3),'Block 4- After opto')
% axis (ax(3),'square')
% 
% 
% %Block5 (2nd opto inactivation)
% 
% arrayfun( @(i) xline( ax(4), Block_relevant_changes_B5(i)+0.5,'--k'), [1:length(Block_relevant_changes_B5)] );                                  % Plot PR2 for Block 3 stimulation
% arrayfun( @(i) plot( ax(4), SubBlockxaxis(Block_5_datapoints), AllPR2_Sess((WichBlockIsStimulated==OptoInBlocks(2)),Block_5_datapoints),"--",'color', [0.9294,0.6941,0.1255],'LineWidth',0.25 ), [1] )     % Plot individual sessions LASER ON
% p1 = arrayfun( @(i) plot( ax(4), SubBlockxaxis(Block_5_datapoints), MeanPR2(2,Block_5_datapoints),'color',[0.9294,0.6941,0.1255],'LineWidth',3), [1] );                                              % Plot mean PR2 LASER ON
% arrayfun( @(i) plot( ax(4), SubBlockxaxis(Block_5_datapoints), AllPR2_Sess((WichBlockIsStimulated==OptoInBlocks(1)),Block_5_datapoints),"--",'color', [0,0,0],'LineWidth',0.05 ), [1] )     % Plot individual sessions LASER OFF
% p2 = arrayfun( @(i) plot( ax(4), SubBlockxaxis(Block_5_datapoints), MeanPR2(1,Block_5_datapoints),'color',[0,0,0],'LineWidth',3), [1] );                                              % Plot mean PR2 LASER OFF
% if Punishment
%     arrayfun( @(i) axis( ax(4), [[SubBlockxaxis(Block_5_datapoints(1)),SubBlockxaxis(Block_5_datapoints(end))] 0.20 0.80]), [1]);
% else
%     arrayfun( @(i) axis( ax(4), [[SubBlockxaxis(Block_5_datapoints(1)),SubBlockxaxis(Block_5_datapoints(end))] 0.00 1.00]), [1]);
% end
% arrayfun( @(i) xticks(ax(4), 0:200:800), [1]);
% 
% if LabelConditions
...
% a = get(gca,'XTickLabel');
% b = get(gca,'YTickLabel');
% ylabel(ax(4),'PR2','fontsize',12)
% title (ax(4),'Block 5- 2nd opto block')
% axis (ax(4),'square')
...
if Punishement
    arrayfun( @(i) plot( ax(1), SubBlockxaxis(Blocks_to_plot)-Trialzero, PR2_Laser_ON_All(1:end,:),"--",'color', [0.9294,0.6941,0.1255],'LineWidth',0.5 ), [1] )     % Plot individual sessions LASER ON
    arrayfun( @(i) plot( ax(1), SubBlockxaxis(Blocks_to_plot)-Trialzero, PR2_Laser_OFF_All(1:end,:),"--",'color', [0,0,0],'LineWidth',0.01 ), [1] )     % Plot individual sessions LASER OFF
    p1 = arrayfun( @(i) plot( ax(1), SubBlockxaxis(Blocks_to_plot)-Trialzero, mean (AllPR2_Sess(WichBlockIsStimulated==2,Block_2_datapoints),1),'color',[0.9294,0.6941,0.1255],'LineWidth',3), [1] );                                              % Plot mean PR2 LASER ON
         arrayfun( @(i) plot( ax(1), SubBlockxaxis(Blocks_to_plot)-Trialzero, mean (AllPR2_Sess(WichBlockIsStimulated==4,Block_4_datapoints),1),'color',[0.9294,0.6941,0.1255],'LineWidth',3), [1] );                                              % Plot mean PR2 LASER ON
    p2 = arrayfun( @(i) plot( ax(1), SubBlockxaxis(Blocks_to_plot)-Trialzero,mean (AllPR2_Sess(WichBlockIsStimulated==4,Block_2_datapoints),1),'color',[0,0,0],'LineWidth',3), [1] );                                              % Plot mean PR2 LASER OFF
         arrayfun( @(i) plot( ax(1), SubBlockxaxis(Blocks_to_plot)-Trialzero,mean (AllPR2_Sess(WichBlockIsStimulated==2,Block_4_datapoints),1),'color',[0,0,0],'LineWidth',3), [1] );                                              % Plot mean PR2 LASER OFF
    
    arrayfun( @(i) axis( ax(1), [[SubBlockxaxis(Blocks_to_plot(1))-Trialzero,SubBlockxaxis(Blocks_to_plot(end))-Trialzero] 0.15 0.85]), [1]);

else
    arrayfun( @(i) plot( ax(1), SubBlockxaxis(Blocks_to_plot)-Trialzero, PR2_Laser_ON_All(1:end,:),"--",'color', [0.9294,0.6941,0.1255],'LineWidth',0.5 ), [1] )     % Plot individual sessions LASER ON
    arrayfun( @(i) plot( ax(1), SubBlockxaxis(Blocks_to_plot)-Trialzero, PR2_Laser_OFF_All(1:end,:),"--",'color', [0,0,0],'LineWidth',0.01 ), [1] )     % Plot individual sessions LASER OFF
    p1 = arrayfun( @(i) plot( ax(1), SubBlockxaxis(Blocks_to_plot)-Trialzero, PR2_Laser_ON_All(end,:),'color',[0.9294,0.6941,0.1255],'LineWidth',3), [1] );                                              % Plot mean PR2 LASER ON
    p2 = arrayfun( @(i) plot( ax(1), SubBlockxaxis(Blocks_to_plot)-Trialzero,PR2_Laser_OFF_All(end,:),'color',[0,0,0],'LineWidth',3), [1] );                                              % Plot mean PR2 LASER OFF
