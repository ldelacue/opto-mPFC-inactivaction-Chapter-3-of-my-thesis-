function [w,nll,A,param,cval,pS2,BIC,raw_data,fitted_data,simulated_data,simulated_response,reward_densities] = kdbfit3x_beta(DataMatrix,model,ConditionLabels,rprobs,StimulusMeans,scale_reward,feedback_mode,...
                                                                                                                        simulation_type,no_update_no_pullback,delta_mapping,trialgroup,trialgroup_def)

% NEW! Implemented new functionality by Luis de la Cuesta to fit different
% learning rates in Laser ON vs OFF trials that happen in alternation such us in the
...
% trials and learning rates are matched, i.e., trialgroup_def.
% This has only been tested by me while I was analysing my data, it
% should be tested by another person, altough results make sense and my
% addition to the code is only two lines (nr. 182-183).  
%                                                       Luis 02/2025
% 
...
%   trialgroup_def:                                                      % NEW!
%     'sequence' : which delta should be used in each trial is defined by DataMatrix(:,1)
%     'condition': which delta should be used in each trial is defined by DataMatrix(:,6)
%     'undefined': which delta should be used in each trial is defined by a
%                  new an additional input, trialgroup_def, provided by the
%                  user.
...
elseif strcmp(trialgroup, 'undefined')    % NEW added by Luis to fit pseudorandom opto presentation (trials which are neither defined by stimulus ID nor by bolck)
    trialgroup = trialgroup_def;
end
...
switch integrator
    case {'leaky integrator', 'leaky integrator on update'}
        ...
        drawnow
        opt = optimset;
        param = fminbnd(@kdb_fit,0.70,1,opt,...   % was 0.01
